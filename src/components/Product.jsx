...
const Product=()=>{
    ...
    <div className="bg-gray-50 w-screen">
        <div className="w-[90%] h-screen flex flex-row mx-auto xss:w-screen xss:grid xss:place-items-center">
            <div className="w-[40%] h-[calc(100%-73px)] xss:w-[70%]">
            ...
            <div className="w-[60%] h-[calc(100%-73px)] mt-[80px] text-gray-600 xss:w-[80%]">
                <h1 className="text-[2rem] xss:text-[1.5rem] font-bold text-purple-600">{product[id]?.title}</h1>
                <h3 className="text-[1.17rem] pb-[10px]">Ratings: {product[id]?.rating?.rate} ({product[id]?.rating?.count})</h3>
                <h4 className="text-[1rem] font-bold pb-[10px]">details:</h4>
                <p className="pb-[22px]">{product[id]?.description}</p>
                <h3 className="text-[2rem] pb-[15px] text-purple-600">{product[id]?.price}$</h3>
                ...
                "  onClick={()=>{dispatch(addToCart(product[id]))}} >Add to cart</button>
